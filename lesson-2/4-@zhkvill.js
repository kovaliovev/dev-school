...
const isItemValide = (item) => item.startsWith('[') && item.endsWith(']');

const isSausagesSame = (sausages) => {
  const firstSausage = sausages[0];
  const len = sausages.length;
  if (len !== 4) return false;
  for (let i = 1; i < len; i++) {
    if (sausages[i] !== firstSausage) return false;
  }
  return true;
};
...
const unpackSausages = (truck) => {
  const result = [];
  let undamagedPackages = 0;

  for (const pack of truck) {
    for (const item of pack) {
      if (!isItemValide(item)) continue;
      const sausages = item.slice(1, -1);

      if (!isSausagesSame(sausages)) continue;
      undamagedPackages++;
      if (undamagedPackages % 5 !== 0) result.push(...sausages);
    }
  }
  return result.join(' ');
};
